...
> 搜索 "600519" 返回贵州茅台。搜索支持股票代码、中文名称（如"茅台"）、拼音首字母（如"GZMT"）三种方式，结果列表点击可跳转个股详情页。
...
![Portal GUEST 首页](screenshots/portal_guest.png)
> Portal 首页（GUEST 视图）：大盘指数面板（Mock 上证/深证）+ 搜索框 + 实时行情列表。未登录时列表仅显示代码、名称、最新价、涨跌幅。
...
![StockDetail STANDARD](screenshots/detial_standard.png)
> 个股详情页（STANDARD 视图）：顶部价格卡片（最新价、涨跌幅、昨收）+ 左侧 K 线图（含 MA5/MA10）+ 右侧盘口面板（买一/卖一价格与挂单量条）+ 主力动向面板（大买家/大卖家账号及成交量）。
...
![StockDetail VIP + MACD](screenshots/detial_vip.png)
> 个股详情页（VIP 视图）：K 线图底部额外展示 MACD 副图——DIF 快线（蓝色）、DEA 慢线（橙色）、MACD 红绿柱。VIP 可自由切换全部 8 个周期（5 分/15 分/30 分/1 时/日/周/月/年）。

![VIP 升级弹窗](screenshots/detail_vip_blur.png)
> VIP 升级弹窗：STANDARD 用户点击非 1D 周期时触发，图表区域高斯模糊（`filter: blur(5px)`），居中弹窗引导升级——锁图标 + "专业版功能" 文案 + "立即升级"按钮 → 第二层模拟支付（手机号 + 验证码 + ¥99.00/月）。
